...
[source,bash,run]
----
mkdir -p /var/www/html/adminer && cd /var/www/html/adminer
----
...
[source,bash,run]
----
# If the file does not already exist, download Adminer
if [ ! -f /var/www/html/adminer/index.php ]; then
    wget -O /var/www/html/adminer/index.php \
...
image::Adminer_MyDatabase.png[Adminer_MyDatabase]
